...
namespace Cadencii.Application.Models
{
	...
	public class PictPianoRollModel
	{
		...
		PictPianoRoll control;

		public PictPianoRollModel (PictPianoRoll control)
		{
		...
		{
			((PictPianoRoll) control).MouseDown += OnMouseDown;
			((PictPianoRoll) control).Paint += OnPaint;
		}
			...
			lock (EditorManager.mDrawObjects) {
				...
				//try {
					...
					if (EditorManager.IsWholeSelectedIntervalEnabled) {
						...
						var mouse = control.PointToClient(new Point(mouse_location.X, mouse_location.Y));
						// 描く四角形の位置とサイズ
