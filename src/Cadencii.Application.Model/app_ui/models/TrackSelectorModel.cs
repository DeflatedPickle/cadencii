...
namespace Cadencii.Application.Models
{
	...
	/// </summary>
	public class TrackSelectorModel : IDisposable
	{
		TrackSelector control;

		public TrackSelectorModel (TrackSelector control)
		{
		...
		public void setEditingPointID (int id)
		{
			mEditingPointID = id;
		}

		public void OnMouseDown (object sender, Cadencii.Gui.Toolkit.MouseEventArgs e)
		{
			onMouseDown (sender, e);
		}

		public void OnMouseUp (object sender, Cadencii.Gui.Toolkit.MouseEventArgs e)
		{
			onMouseUp (sender, e);
		}
		...
		public Cadencii.Gui.Point LocationOnScreen {
			get { return control.PointToScreen (control.Location); }
		}
		...
		private void executeCommand(CadenciiCommand command, bool register)
		{
			...
			try {
				if (CommandExecuted != null) {
					CommandExecuted.Invoke(control, new EventArgs());
				}
			} catch (Exception ex) {
				...
				if (!old.equals (mSelectedCurve)) {
					...
					try {
						if (SelectedCurveChanged != null) {
							SelectedCurveChanged.Invoke (control, mSelectedCurve);
						}
					} catch (Exception ex) {
			...
			if (mLastPreferredMinHeight != min_size) {
				try {
					if (PreferredMinHeightChanged != null) {
						PreferredMinHeightChanged.Invoke(control, new EventArgs());
					}
				} catch (Exception ex) {
		...
		public void TrackSelector_Load(Object sender, EventArgs e)
		{
			...
			control.UserPaint = true;
			control.AllPaintingInWmPaint = true;
		}
			...
			if (CurveVisible) {
				if (e.Button == Cadencii.Gui.Toolkit.MouseButtons.Left) {
					...
					if (0 <= e.X && e.X <= EditorManager.keyWidth &&
						...
						cmenuCurve.Show(control, e.X, e.Y);
					}
				}
			}
		}
									...
									if (EditorManager.Selected != new_selected) {
										...
										try {
											if (SelectedTrackChanged != null) {
												SelectedTrackChanged.Invoke(control, i + 1);
											}
										} catch (Exception ex) {
										...
										if (EditorManager.getRenderRequired(EditorManager.Selected) && !EditorManager.isPlaying()) {
											try {
												if (RenderRequired != null) {
													RenderRequired.Invoke(control, EditorManager.Selected);
												}
											} catch (Exception ex) {
												Logger.StdErr("TrackSelector#TrackSelector_MouseDown; ex=" + ex);
											}
		...
		private void changeCurve(CurveType curve)
		{
			...
			if (!mSelectedCurve.equals(curve)) {
				...
				try {
					if (SelectedCurveChanged != null) {
						SelectedCurveChanged.Invoke(control, curve);
					}
				} catch (Exception ex) {
				...
				if (0 <= e.Y && e.Y <= height - 2 * TS.OFFSET_TRACK_TAB) {
					...
					if (key_width <= e.X && e.X <= width) {
						if (!mSelectedCurve.equals(CurveType.VEL) &&
							...
							if (found) {
								...
								try {
									fbpe = ApplicationUIHost.Create<FormBezierPointEditUi> (control, mSelectedCurve, chain_id, target_point.getID());
					...
					if (EditorManager.SelectedTool != EditTool.ERASER) {
						...
						if (ve != null) {
							...
							cmenuSinger.Show(control, e.X, e.Y);
		...
		public void cmenusinger_MouseHover(Object sender, EventArgs e)
		{
			try {
				...
				Rectangle rrc = Cadencii.Gui.Toolkit.Screen.Instance.getScreenBounds(control);
		...
		private void registerEventHandlers()
		{
			...
			control.Load += new EventHandler(this.TrackSelector_Load);
		...
		private void InitializeComponent()
		{
			...
			this.cmenuCurve = ApplicationUIHost.Create<UiContextMenuStrip>(this.components);
			this.cmenuCurveVelocity = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveAccent = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveDecay = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveSeparator1 = ApplicationUIHost.Create<UiToolStripSeparator>();
			this.cmenuCurveDynamics = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveVibratoRate = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveVibratoDepth = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveSeparator2 = ApplicationUIHost.Create<UiToolStripSeparator>();
			this.cmenuCurveReso1 = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso1Freq = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso1BW = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso1Amp = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso2 = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso2Freq = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso2BW = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso2Amp = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso3 = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso3Freq = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso3BW = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso3Amp = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso4 = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso4Freq = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso4BW = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveReso4Amp = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveSeparator3 = ApplicationUIHost.Create<UiToolStripSeparator>();
			this.cmenuCurveHarmonics = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveBreathiness = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveBrightness = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveClearness = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveOpening = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveGenderFactor = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveSeparator4 = ApplicationUIHost.Create<UiToolStripSeparator>();
			this.cmenuCurvePortamentoTiming = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurvePitchBend = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurvePitchBendSensitivity = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveSeparator5 = ApplicationUIHost.Create<UiToolStripSeparator>();
			this.cmenuCurveEffect2Depth = ApplicationUIHost.Create<UiToolStripMenuItem>();
			this.cmenuCurveEnvelope = ApplicationUIHost.Create<UiToolStripMenuItem>();
