	...
	public partial class FormMainModel
	{
		public class PictPianoRollModel
		{
				...
				if (e.Button == NMouseButtons.Left) {
					...
					if (item != null &&
						...
						if (EditorManager.SelectedTool == EditTool.ERASER) {
							...
							int stdx = parent.form.controller.getStartToDrawX();
							int stdy = parent.form.controller.getStartToDrawY();
							for (int i = 0; i < EditorManager.mDrawObjects[selected - 1].Count; i++) {
								DrawObject dobj = EditorManager.mDrawObjects[selected - 1][i];
								if (dobj.mRectangleInPixel.X + parent.form.controller.getStartToDrawX() + dobj.mRectangleInPixel.Width - stdx < 0) {
									continue;
					...
					if (EditorManager.editorConfig.ShowExpLine && EditorManager.keyWidth <= e.X) {
						...
						foreach (var dobj in EditorManager.mDrawObjects[selected - 1]) {
							...
							rect = new Rectangle(
								...
								dobj.mRectangleInPixel.Y - stdy + (int)(100 * parent.form.controller.getScaleY()),
								21,
								(int)(100 * parent.form.controller.getScaleY()));
					...
					if ((modefier & parent.form.s_modifier_key) == parent.form.s_modifier_key) {
						...
						if (selected_tool == EditTool.LINE || selected_tool == EditTool.PENCIL) {
							...
							if (vibrato_dobj != null) {
								...
								EditorManager.mAddingEvent.ID.VibratoDelay = length - (int)(px_vibrato_length * parent.form.controller.getScaleXInv());
						...
						if (vibrato_dobj == null) {
							if ((selected_tool == EditTool.PENCIL || selected_tool == EditTool.LINE) &&
								...
								if (MusicManager.getVsqFile().getPreMeasureClocks() - EditorManager.editorConfig.PxTolerance * parent.form.controller.getScaleXInv() <= clock) {
									//10ピクセルまでは許容範囲
			...
			public void RunPianoRollMouseMove(MouseEventArgs e)
			{
				lock (EditorManager.mDrawObjects) {
					...
					if (mExtDragX == ExtDragXMode.RIGHT || mExtDragX == ExtDragXMode.LEFT) {
						...
						float inv_scale_x = parent.form.controller.getScaleXInv();
					...
					if (edit_mode == EditMode.MIDDLE_DRAG) {
						...
						if (unit * parent.form.controller.getScaleX() > 10) { //これをしないと、グリッド2個分増えることがある
			...
			public void RunPianoRollMouseUp(MouseEventArgs e)
			{
				...
				int track_height = (int)(100 * parent.form.controller.getScaleY());
				...
				if ((modifier & Keys.Control) == Keys.Control) {
					...
					if (horizontal) {
						...
						if (old_tbv != draft) {
							...
							// 古い拡大率
							float scale0 = parent.form.controller.getScaleX();
							// 新しい拡大率
							float scale1 = parent.form.getScaleXFromTrackBarValue(draft);
							// 古いstdx
							int stdx0 = parent.form.controller.getStartToDrawX();
							...
							parent.form.controller.setScaleX(scale1);
							parent.form.controller.setStartToDrawX(stdx1);
