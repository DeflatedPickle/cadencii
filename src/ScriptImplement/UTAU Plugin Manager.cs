...
class UtauPluginManager : Form
{
            ...
            if (tname == name) {
                var dr =
                    DialogManager.ShowMessageBox(string.Format(_("Script named '{0}' is already registered. Overwrite?"), name),
						...
						Cadencii.Gui.Toolkit.MessageBoxIcon.Question);
    ...
    private void btnOk_Click(object sender, EventArgs e)
    {
        if (oldPlugins != null) {
            ...
            foreach (string file in oldPlugins) {
                if (!getPlugins().Contains(file)) {
                    string name = getPluginName(file);
                    if (name != "") {
                        string script_path = Path.Combine(EditorManager.getScriptPath(), name + ".txt");
        ...
        foreach (string file in getPlugins()) {
            ...
            using (StreamWriter sw = new StreamWriter(Path.Combine(EditorManager.getScriptPath(), name + ".txt"))) {
                sw.WriteLine(code);
            }
        }
